...
export default function Queries() {
  const { queryResults, refreshData } = useQueryData();
  ...
  const avgPosition = queryResults
    .filter((q) => q.position)
    .reduce((acc, q) => acc + (q.position || 0), 0) / queryResults.filter((q) => q.position).length || 0;
  const positiveCount = queryResults.filter((q) => q.sentiment === 'positive').length;
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <KPICard
          label="Mention Rate"
          value={queryResults.length > 0 ? Math.round((mentionedCount / queryResults.length) * 100) : 0}
          change={5.2}
          trend="up"
          description={`${mentionedCount} of ${queryResults.length} monitored queries`}
          suffix="%"
        ...
        <KPICard
          ...
          value={mentionedCount > 0 ? Math.round((positiveCount / mentionedCount) * 100) : 0}
          change={2.1}
          trend="up"
